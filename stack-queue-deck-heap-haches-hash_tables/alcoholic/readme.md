...
**Входные данные**  
Программа получает на вход три строки. В первой строке содержится целое чётное число n ($2 \leq n \leq 100 000). Вторая строка содержит  чисел — карты первого игрока, а третья —  карт второго игрока. Карты перечислены сверху вниз, то есть каждая строка начинается с той карты, которая будет открыта первой. Гарантируется, что каждая из карт встречается в колодах игроков ровно один раз.

**Выходные данные**  
Программа должна определить, кто выигрывает при данной раздаче, и вывести слово «first» или «second», после чего вывести количество ходов, сделанных до выигрыша. Если на протяжении $*10^5$ ходов игра не заканчивается, программа должна вывести слово «draw».
